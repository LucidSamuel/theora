import type { CompilationResult, WitnessResult, CheckResult, FailureTrace, ConstraintAnalysis } from '@/lib/dsl/types';
...
export interface DebugRenderState {
  ...
  /** When present, wires flagged unconstrained/weak are tinted on the canvas. */
...
export function renderDebugGraph(
  ...
  // Check status map (empty when no witness/checks yet)
  ...
  // Wires flagged by the analyzer as unconstrained or never reaching a
  // multiplication — the prover can pick their values freely.
  const flaggedWireIds = new Set<number>();
  if (state.analysis) {
    for (const wire of state.analysis.unconstrainedWires) flaggedWireIds.add(wire.id);
    for (const wire of state.analysis.weakInputWires) flaggedWireIds.add(wire.id);
  }
  ...
  for (const [wireId, pos] of layout.wirePositions) {
    ...
    const isFlagged = flaggedWireIds.has(wireId);
    ...
    if (isFlagged) strokeColor = c.wireUnconstrained;
    ...
    if (isFlagged && !isFailed) ctx.setLineDash([4, 3]);
    ctx.stroke();
    ctx.setLineDash([]);
    ...
    if (value !== undefined) {
      ...
      ctx.fillStyle = c.wireUnconstrained;
      ctx.font = '10px var(--font-mono, monospace)';
