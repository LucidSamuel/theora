...
import { applyImportedState, fetchTheoraImport, parseTheoraImport, resolveGitHubImportSource, getCurrentExportEnvelope, serializeTheoraImport } from '@/lib/githubImport';
...
describe('same-demo import regression', () => {
  it('derives the active demo from search params when the hash is absent', () => {
    window.history.replaceState(null, '', '/app?pl=encoded-pipeline-state');

    expect(getActiveDemoLocation()).toEqual({
      activeDemo: 'pipeline',
      locationKey: 'pipeline:encoded-pipeline-state',
    });
  });
